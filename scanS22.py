...
# 実行順序（allモード時）
# ============================================================
MODE_ORDER = ["s22flint", "s22", "flint", "naive"]
...
def load_config():
    ...
    if not os.path.exists(CONFIG_PATH):
        with open(CONFIG_PATH, "w", encoding="utf-8") as f:
            ...
            f.write("#\n")
            f.write("# mode の選択肢:\n")
            f.write("#   s22flint : S_{(2,2)}分解 + FLINT x2  （最速）\n")
            f.write("#   s22      : S_{(2,2)}分解 + naive x2\n")
            f.write("#   flint    : 一般点 FLINT\n")
            f.write("#   naive    : 一般点 打ち切り級数       （最遅）\n")
            f.write("#   all      : 上記を s22flint→s22→flint→naive の順に実行\n")
            f.write("#\n")
...
def make_block_omegas(g_total, seed=42):
    """
    S_{(2,2)}分解用：g_total を半分に分けた2つの周期行列を返す
...
# FLINT Theta計算
# ============================================================
def theta_flint_single(z, Omega):
    """1つの周期行列に対してFLINTでTheta計算"""
    try:
        from flint import acb_mat
        # TODO: API確認後に実装
        raise NotImplementedError("FLINT API要確認・実装後に差し替え")
    except ImportError:
        raise ImportError("pip install python-flint が必要です")

# ============================================================
# 4モードの実行
# ============================================================
def run_s22flint(g, N_cut, seed, report_every):
    """S_{(2,2)}分解 + FLINT x2（最速）"""
    ...
    start = time.perf_counter()
    t1 = theta_flint_single(z1, Omega1)
    t2 = theta_flint_single(z2, Omega2)
    result = t1 * t2
    return result, time.perf_counter() - start
...
def run_flint(g, N_cut, seed, report_every):
    """一般点 FLINT（分解なし）"""
    Omega = make_omega(g, seed)
    z = np.random.default_rng(seed).standard_normal(g)
    start = time.perf_counter()
    result = theta_flint_single(z, Omega)
    return result, time.perf_counter() - start
...
def print_summary(all_results):
    ...
    for N_cut in [1, 2]:
        for g in [5, 10, 17]:
            ...
            if "naive" in times and "s22" in times:
                ratio = times["naive"] / times["s22"]
                log(f"  g={g} N={N_cut}: "
                    f"naive/s22={ratio:.1f}倍  "
                    + (f"flint/s22flint={times['flint']/times['s22flint']:.1f}倍"
                       if "flint" in times and "s22flint" in times else ""))
...
def main():
    ...
    mode = config["mode"]
    if mode == "all":
        modes_to_run = MODE_ORDER  # [s22flint, s22, flint, naive]
    elif mode in RUNNERS:
        modes_to_run = [mode]
    else:
        log(f"不明なmode: {mode}")
        log(f"使用可能: {list(RUNNERS.keys()) + ['all']}")
        sys.exit(1)
    ...
    for current_mode in modes_to_run:
        ...
        for g in g_list:
            for N_cut in N_cut_list:
                ...
                try:
                    runner = RUNNERS[current_mode]
                    val, elapsed = runner(g, N_cut, seed, report_every)
