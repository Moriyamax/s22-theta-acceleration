...
## ベンチマーク・検証用途
高次元のtheta関数は実用上、近似的に扱われますが、本実装はそれらの近似の妥当性を検証するための基準値として利用できます。
特に：
- 近似式の誤差評価
- 漸近展開の検証
- 従来は直接計算が困難だった領域（g > 20）での数値クロスチェック
を可能にします。
これにより、数理物理・可積分系の数値的研究におけるベンチマークツールとして利用できると考えます。
...
s22（8+9 次元）実測: 13.1 秒 (CPU:Ryzen 5 5500GT)

約 10^5 倍の高速化になります。
--

1.特徴
s22 と naive の 2 モードを実装
